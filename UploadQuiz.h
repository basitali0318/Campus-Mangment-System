...
#ifndef OOP_NEW_PROJECT_UPLOADQUIZ_H
#define OOP_NEW_PROJECT_UPLOADQUIZ_H
...
class UploadQuiz {
public:
    void uploadQuiz(const string& enrollment) {
        ...
        int choice;
        cout << "Choose a course to upload quiz: ";
        ...
        // Upload the quiz for the selected course
        ...
        if (GetOpenFileName(
                &ofn)) // Display the Open dialog box. If the user selects a file and clicks OK, the function returns true.
        {
            ...
            string::size_type pos = string(buffer).find_last_of(
                    "\\/"); // Find the last occurrence of a slash in the path.
            string quizPath = string(buffer).substr(0, pos) + "/" + enrollment + "_" + courseCode +"_Quiz"; // Construct the path for the new file.
            ofstream dstFile(quizPath, ios::binary); // Open the new file in binary mode for writing.
            ...
            cout << "Quiz uploaded successfully."
                 << endl; // Print a message to indicate the successful completion of the upload process.
        } else {
            cout << "No file selected." << endl; // Print a message if no file was selected in the dialog box.
        }
    }
};
#endif //OOP_NEW_PROJECT_UPLOADQUIZ_H
